...
기본 구현 도구는 `Claude`이며, Expo/React Native 관련 요청에서는 문서 규칙에 따라 대응 `SKILL.md`를 먼저 확인합니다.
Expo 리스크가 큰 변경에는 관련 스킬을 먼저 확인합니다. 필요한 경우 `Codex`를 보조 도구로 활용할 수 있습니다. `Gemini`는 보조 검토 또는 대체 검토 도구로 사용합니다.
...
### 스킬 확인 규칙
- 코드 구조 정리, 리팩터링, 커스텀 훅 분리, 가독성 개선, Thin Screen 작업: `.agents/skills/dasida-code-structure/SKILL.md`
...
### 적용 방식
- 기본 구현은 `Claude`가 수행합니다.
- 관련 작업에서는 대응 `SKILL.md`와 필요한 `references/`를 먼저 확인합니다.
- 코드 구조/리팩터링 작업은 `docs/ARCHITECTURE.md`와 `.agents/skills/dasida-code-structure/SKILL.md`를 기준으로 진행합니다.
- 스킬 확인은 권장 규칙이며 단순 수정에는 생략할 수 있습니다.
