...
describe('buildDiagnosisQueue', () => {
  ...
  it('일부 진단 완료 — 미진단만, 클릭한 문제 첫 자리', () => {
    const progress: ExamDiagnosisProgress = { 5: 'formula_understanding' as WeaknessId };
    expect(buildDiagnosisQueue([1, 3, 5], progress, 3)).toEqual([3, 1]);
    expect(buildDiagnosisQueue([1, 3, 5], progress, 1)).toEqual([1, 3]);
  });
  ...
  it('마지막 한 개만 미진단 + 그것 클릭', () => {
    ...
    expect(buildDiagnosisQueue([1, 3, 5], progress, 1)).toEqual([1]);
  });

  it('이미 진단 완료된 문제 클릭 — 빈 배열 (방어)', () => {
    const progress: ExamDiagnosisProgress = { 3: 'calc_repeated_error' as WeaknessId };
    expect(buildDiagnosisQueue([1, 3, 5], progress, 3)).toEqual([]);
  });
