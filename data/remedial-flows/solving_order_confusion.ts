...
export const solving_order_confusion_flow: RemedialFlow = {
  nodes: {
    ...
    'soc_step1_A_explain': {
      ...
      body: "f(x) 는 원래 식, f'(x) (작은 따옴표 하나 붙은 것) 는 그 식을 미분해서 만든 기울기 계산식이에요. f(x)=0 은 그래프가 x 축에 닿는 자리 (영점) 를 구하는 식이고, f'(x)=0 은 그래프 기울기가 0 인 자리 (극값 후보 — 그래프가 잠깐 멈추는 가장 높거나 낮은 점) 를 구하는 식이에요. 두 자리는 보통 달라요.",
      primaryLabel: '다음으로',
    ...
    'soc_step1_A_check': {
      ...
      kind: 'check',
      title: '확인 문제',
      prompt: 'f(x) = x² − 2x 의 극값 후보 (그래프 기울기가 0 인 자리) x 를 구하려면 어떤 식을 0 으로 놓아야 할까요?',
      options: [
        { id: 'correct', text: "f'(x) = 2x − 2 = 0", isCorrect: true, nextNodeId: 'soc_step1_exit' },
        { id: 'wrong1',  text: 'f(x) = x² − 2x = 0', isCorrect: false, nextNodeId: 'soc_step1_A_remedy', weaknessId: 'basic_concept_needed' },
        { id: 'wrong2',  text: 'f(0) = 0', isCorrect: false, nextNodeId: 'soc_step1_A_remedy', weaknessId: 'basic_concept_needed' },
      ],
    ...
    'soc_step1_C_explain': {
      ...
      body: '원래 식 f(x) 에 x=0 을 넣으면 그건 그래프가 세로축과 만나는 시작값이에요 (이걸 y 절편이라 불러요 — y 축과의 만남 점). 극값(그래프가 가장 높거나 낮아지는 자리) 과는 다른 자리예요. 극값을 찾으려면 기울기 식 f\'(x) 를 0 으로 놓고 풀어야 해요.',
      primaryLabel: '다음으로',
    ...
    'soc_step1_C_check': {
      ...
      kind: 'check',
      title: '확인 문제',
      prompt: 'f(x) = x² − 4x + 1 의 극값 후보 (그래프 기울기가 0 인 자리) x 를 구하려면 어떤 식을 0 으로 놓을까요?',
      options: [
    ...
    'soc_step2_A_remedy': {
      ...
      kind: 'explain',
      title: '± 까지 챙겨요',
      body: 'x² = 4 라면 x = 2 와 x = −2 둘 다 답이에요. 둘을 모두 적어두면 한쪽에서 그래프가 위로 꺾이는 자리(극댓값) 와 다른 쪽에서 아래로 꺾이는 자리(극솟값) 를 둘 다 검토할 수 있어요.',
      primaryLabel: '다음으로',
      primaryNextNodeId: 'soc_step2_A_check',
      secondaryLabel: '모르겠어요',
      secondaryNextNodeId: 'soc_step2_exit',
    ...
    'soc_step2_C_explain': {
      ...
      kind: 'explain',
      title: '음수 해도 후보예요',
      body: '극값(그래프가 잠깐 멈추는 자리) 은 그래프 어디서든 생길 수 있어요. x 가 음수인 자리에서도 그래프가 위·아래로 꺾일 수 있다는 뜻이에요. "x 는 양수만 답이다" 같이 미리 정하면 진짜 답 한쪽을 통째로 놓쳐요.',
      primaryLabel: '다음으로',
    ...
    'soc_step3_C_check': {
      ...
      kind: 'check',
      title: '확인 문제',
      prompt: 'f(x) = x² − 4x + 1 에서 x = 2 가 극값 후보예요. 극값 (그 자리의 그래프 높이) 은?',
      options: [
        { id: 'correct', text: 'f(2) = 4 − 8 + 1 = −3', isCorrect: true, nextNodeId: 'soc_step3_exit' },
        { id: 'wrong1',  text: 'x = 2', isCorrect: false, nextNodeId: 'soc_step3_C_remedy', weaknessId: 'basic_concept_needed' },
    ...
    'soc_step3_C_remedy': {
      ...
      kind: 'explain',
      title: '대입해서 −3 이 답',
      body: 'f(2) = 4 − 8 + 1 = −3. x = 2 는 그저 그래프의 가로 자리 표시고, 그 자리의 그래프 높이 −3 이 진짜 극값이에요 (이 경우 그래프가 가장 낮게 내려간 값).',
      primaryLabel: '다음으로',
      primaryNextNodeId: 'soc_step3_C_check',
      secondaryLabel: '모르겠어요',
      secondaryNextNodeId: 'soc_step3_exit',
