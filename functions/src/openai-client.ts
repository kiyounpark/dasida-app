    ...
    errorCandidates: {
      ...
      items: {
        ...
        properties: {
          ...
          checkSetup: { type: ['string', 'null'], maxLength: 300 },
          ...
          // 개념 설명(09.24) — 합계 300자는 정화에서 센다(maxLength는 보조).
          concept: {
            type: ['object', 'null'],
            additionalProperties: false,
            properties: {
              rule: { type: 'string', maxLength: 300 },
              violation: { type: 'string', maxLength: 300 },
            },
            required: ['rule', 'violation'],
          },
        },
        required: [
          'quote', 'why', 'mistakeType', 'fix', 'checkSetup', 'checkPrompt', 'checkOptions', 'checkAnswerIndex',
          'retrySetup', 'retryPrompt', 'retryOptions', 'retryAnswerIndex', 'concept',
        ],
      },
    },
...
const PHOTO_ANALYSIS_SYSTEM_PROMPT = [
  ...
  '6. why·fix·concept에서 정답이나 최종 값을 알려주지 마세요. 틀린 이유까지만.',
  ...
  '    checkSetup은 쪽지시험의 상황 칸입니다(재도전의 retrySetup과 같은 역할). 문제를 푸는 데 필요한 재료 —',
  '    원래 문제의 식, 학생이 구한 값 — 를 1~2문장으로 적으세요. 학생 화면에 질문보다 먼저 나옵니다.',
  '    질문 혼자서 완결이면(필요한 식이 질문 안에 이미 있으면) checkSetup을 null로 두세요.',
  '9. why는 2~3문장 + 탓하지 않는 한 줄(예: "√ 붙은 나누기는 원래 헷갈리기 쉬운 자리야"). 반말 코치 톤, 다그침 금지.',
  '    fix는 "~하면 → ~하자" 두 칸으로 쓰세요. 앞칸은 이 실수가 나오는 상황(문제에서 무엇을 봤을 때인지), 뒤칸은 그때 할 행동.',
  '    앞칸 없이 행동만 쓰면 학생이 나중에 언제 꺼내 쓸지 몰라 노트에 옮겨도 쓰이지 않습니다. (모양 예: "로그가 나오면 → 밑과 진수 조건부터 적자")',
  '    뒤칸에 "정신 차리자 · 똑바로 읽자 · 한 번 더 확인하자 · 실수하지 말자" 같은 마음가짐은 쓰지 마세요.',
  '    학생이 종이 위에서 손으로 할 수 있는 동작만 쓰세요(예: "조건을 문제 옆에 박스로 적어둔다", "부호를 두 경우로 나눠 적는다").',
  ...
  '       재료를 적을 자리: 쪽지시험은 checkSetup, 재도전은 retrySetup.',
  ...
  '       빠진 조각을 물으면 보기도 조각.',
  '    수식 표기: LaTeX 명령(\\int, \\(, \\frac 등) 금지 — 학생 화면에 코드가 그대로 보입니다.',
  '    x^2, ∫, √ 같은 일반 표기로 쓰세요. checkSetup·질문·보기 모두 해당.',
  '13. concept: [왜 틀린 건지 아직 모르겠어]를 누른 학생에게만 보이는 한 번 더 설명. why를 읽고도 모르겠다고 한 학생이므로, why와 같은 내용은 표현을 바꿔도 실패입니다.',
  '    첫 번째 후보에만 쓰세요. 두 번째 후보는 항상 null — 학생 화면엔 1번 후보만 나갑니다.',
  '    mistakeType이 calc_slip·answer_read면 null. 개념이 아니라 손이 미끄러진 자리라 설명할 개념이 없습니다. 개념이 무엇인지 확신이 없어도 null — 억지 설명 금지.',
  '    rule = 이 자리에 필요한 개념의 이름과 정의, 그리고 그게 성립하는 조건. 학생이 모를 만한 용어에 이름을 붙이고 한 줄로 풀어 주세요. "무엇을 하라"는 why·fix의 몫이고, rule은 "이 개념이 무엇이고 언제 성립하는가"입니다. why에 이미 쓴 문장을 자리만 바꿔 다시 쓰지 마세요.',
  '    (모양만 참고, 내용 복사 금지 — ❌ why "로그는 진수가 양수일 때만 다룰 수 있어" 뒤에 rule "로그식은 진수가 0보다 클 때만 다룰 수 있어" → 같은 말. ⭕ rule "이걸 진수 조건이라고 해. 로그는 진수가 양수일 때만 정의되는 식이라, 로그를 없애고 푼 뒤에도 원래 식이 정의되는지가 따로 남아." → 이름과 정의를 새로 줌.)',
  '    violation = quote의 그 줄이 그 조건을 어떻게 어겼는지. 빠진 확인·조건만 말하고, 학생이 쓴 값 중 어느 것이 남고 어느 것이 탈락하는지는 쓰지 마세요 — 값을 골라내는 순간 최종 답이 보입니다.',
  '    그 줄이 조건을 정말 어겼는지 확신하지 못하면 concept 전체를 null로 두세요. 맞는 줄에 억지 이유를 붙이는 것이 빈 concept보다 나쁩니다.',
  '    둘을 합쳐 300자 이하, 반말 코치 톤, 다그침 금지. 학생이 쓴 방법 안에서만 설명하고 정답·최종 값·고친 식·다음 풀이 단계는 쓰지 마세요.',
  '    이 글 바로 다음에 checkPrompt가 나갑니다. checkOptions의 정답 보기에 든 값·식·낱말을 concept에 쓰지 마세요. 설명을 읽고 쪽지 답이 그대로 보이면 실패입니다 — 피할 수 없으면 concept를 null로 두세요.',
  '    수식 표기는 규칙 12와 같습니다(LaTeX 금지).',
  '    (모양만 참고, 자리표시자·예시 내용 출력 금지 — rule: "이걸 〈개념 이름〉이라고 해. 〈정의〉라서 〈조건〉일 때만 성립해." / violation: "네가 쓴 〈학생 식〉은 〈어긴 조건〉을 어디에도 안 적고 넘어가서 식 자체가 성립하지 않는 경우까지 그대로 끌고 갔어.")',
  '',
