...
- 원격: origin
- 원격 URL: https://github.com/kiyounpark/dasida-app.git
- 링크: https://github.com/kiyounpark/dasida-app/commit/d058f017ccfb228cda840f2388f4844ca1aa498a
- 작성자: 박기윤
- 메시지: docs: STATUS 1.0.9 두 스토어 심사 제출 ✅ — E는 심사 중
...
- 원격: origin
- 원격 URL: https://github.com/kiyounpark/dasida-app.git
- 링크: https://github.com/kiyounpark/dasida-app/commit/f707447e12702f5ea87f9b964f5b0744ae9a5879
- 작성자: 박기윤
- 메시지: docs: STATUS 순서·목표 목록에도 사용량 로그 ✅ 표시
- 본문: Co-Authored-By: Claude Opus 5.5 <noreply@anthropic.com>
...
- 원격: origin
- 원격 URL: https://github.com/kiyounpark/dasida-app.git
- 링크: https://github.com/kiyounpark/dasida-app/commit/8743539980e7930eba9c04be71b056b3e7c87e0d
- 작성자: 박기윤
- 메시지: docs: STATUS 헤더 해시 (6d5d6ae)
- 본문: Co-Authored-By: Claude Opus 5.5 <noreply@anthropic.com>

### 커밋 2026.09.23 16:46
- 해시: `701bdb5` (`701bdb5b1cb46d50abe6645ff8f68ed85b544a38`)
- 브랜치: main
- 원격: origin
- 원격 URL: https://github.com/kiyounpark/dasida-app.git
- 링크: https://github.com/kiyounpark/dasida-app/commit/701bdb5b1cb46d50abe6645ff8f68ed85b544a38
- 작성자: 박기윤
- 메시지: docs: STATUS 헤더 해시 (bc42a62)
- 본문: Co-Authored-By: Claude Opus 5.5 <noreply@anthropic.com>
...
- 브랜치: weakness-label
### 커밋 2026.09.20 17:00
- 해시: `5282b08` (`5282b08562d2db8224280fe9e42137840e6bdb2a`)
- 브랜치: main
- 원격: origin
- 원격 URL: https://github.com/kiyounpark/dasida-app.git
- 링크: https://github.com/kiyounpark/dasida-app/commit/5282b08562d2db8224280fe9e42137840e6bdb2a
- 작성자: 박기윤
- 메시지: feat(hooks): 방향 질문 게이트 — "새 숫자 있어요?"를 먼저 묻게 한다
- 본문: 🧭 09.15 🔒에 "기윤이 방향 얘기를 꺼내면 Claude가 먼저 '새 숫자 있나?'를 / 묻는다(기억할 것 0개)"가 잠겨 있는데, 09.20에 Claude가 그걸 어겼다. / 기억에 기대는 장치라서다. 그래서 기억을 훅으로 옮긴다. / - UserPromptSubmit에 direction-model-gate.mjs 추가. 기존 Expo 훅은 안 건드림 / - 하는 일: Fable을 자동 호출하지 않는다. Claude에게 "답하기 전에 새 숫자를 / 물어라"를 꽂을 뿐이다. 실제 호출은 기윤 승낙 뒤 (Fable은 울트라코드와 / 주간 한도를 나눠 먹는다) / - 값의 본체는 Fable이 아니라 "숫자 없이 방향 대화를 시작하는 것"을 막는 것. / 09.14에 그래서 하루가 날아갔다 / - 트리거: "사람 어떻게 모으지 / 안 먹히나 / 다음 글 뭐 쓸까" 등 단독어 + / (오르비·마케팅·방향·조회·좋아요 등 주제어 × 정할까·고를까·어떻게 할까) / - "마케팅" 단독 발동은 뺐다 — 첫 발동이 "마케팅 훅을 어떻게 만들까"에 걸린 / 오탐이었다. 도구 얘기에도 그 단어가 나온다 / - 세션당 2회까지. 1회면 오탐 한 번에 기회가 사라진다 / 테스트: 걸려야 하는 4문장 전부 발동 / 안 걸려야 하는 3문장(오탐 문장 포함) / 전부 미발동 / 같은 세션 3번째 미발동. settings.json 문법 확인. / Co-Authored-By: Claude Opus 5 <noreply@anthropic.com>
...
- 원격: origin
- 원격 URL: https://github.com/kiyounpark/dasida-app.git
- 링크: https://github.com/kiyounpark/dasida-app/commit/3ef46ce2ad313d339bf914ed63dd13b03a0d412b
- 작성자: 박기윤
- 메시지: fix: 학생한테 14일 나간 뒤집힌 수학 한 줄 (볼록 방향과 최대·최소)
- 본문: data/detailedDiagnosisFlows.ts:166 `diff_judge`의 body가 이렇게 나가고 있었다: / "위로 볼록이면 최솟값, 아래로 볼록이면 최댓값이에요." / 둘 다 반대다. a > 0 이면 아래로 볼록(∪)이라 최솟값을, a < 0 이면 위로 볼록(∩)이라 / 최댓값을 가진다. / 하필 같은 항목의 remedialBody는 "a > 0이면 최솟값, a < 0이면 최댓값"으로 맞게 / 적혀 있었다 — 한 화면 안에서 두 문장이 서로 반대말을 하고 있었다는 뜻이다. / 미분에서 최대·최소 판단이 막힌 학생에게 나가는 자리라, 정확히 그게 헷갈리는 / 학생이 본다. / 2026.08.13에 발견해 "남은 것"에 적어뒀고, 08.25에 "12일째 유저에게 나가는 중"으로 / 다시 적혔다. 오늘로 14일째다. / 회귀 테스트 3개(data/detailed-diagnosis-flows-copy.test.ts) — 뒤집힌 문장이 남아 / 있으면 울고, 볼록 방향 설명과 a 부호 기준이 서로 어긋나도 운다. / 타입 검사도 기존 테스트 564개도 이 종류는 못 잡는다. 문장이 참인지는 사람이 / 읽어야 알고, 사람은 12일을 지나쳤다. / 참고: web-proto/flow-bundle.js에는 이 흐름이 안 담겨 있어 웹은 무영향이다. / 다만 그 번들 자체가 08.13에 멈춰 있어 59번째 약점("지수법칙·밑 통일 오류")이 / 웹에 0건인 건 그대로다 — 별건. / 검증: typecheck 0 · jest 567 test 전부 통과(564 → 567). / Co-Authored-By: Claude Opus 5 <noreply@anthropic.com>

### 커밋 2026.08.27 21:11
- 해시: `8c2afa8` (`8c2afa83529cbd4c309a70c3fff48b014483dae5`)
- 브랜치: main
- 원격: origin
- 원격 URL: https://github.com/kiyounpark/dasida-app.git
- 링크: https://github.com/kiyounpark/dasida-app/commit/8c2afa83529cbd4c309a70c3fff48b014483dae5
- 작성자: 박기윤
- 메시지: docs: 오르비 3편(11호) 초안 v1~v9
- 본문: 어제 v2.1에서 오늘 v9까지. 기윤이 잡은 게 대부분이고, 각 판에 왜 고쳤는지를 남겼다. / - v3: first-reader 재검수 6건 — 첫 줄이 "지난 편" 메타라 3초를 버린 것, / "3번에서 멈췄음" 번호 라벨로 눈이 리스트로 되돌아간 것, 편↔명 단위 불일치, / 각주 2건짜리 서술을 학생 댓글 원문으로 교체 / - v4: "지난 편에서 다섯 편이…" 문단 통삭제(기윤). 본문에 지난 편 언급 0회가 됨 / - v5: 🔑 원문을 업고 있는 문장 — 그 줄만 떼서 읽으면 안 읽히는 줄들(기윤). / "진짜로 종이를 접음"·"그딴거 없습니다"가 원문 읽은 사람 머릿속에서만 풀렸다. / 10조 따옴표 계약이 이걸 금지하는데 검수가 "원문 그대로인가"만 대조했다 / - v6: 원문 재크롤로 접기 = 페이지 귀퉁이 접기 확정(댓글 "두번째 틀린거는 위로 / 접으면 돼요" 좋아요 8이 근거). 등판 다리에 영수증 추가 — 그 글 댓글에 / "틀린 이유를 모르면 접은 다음에 어케 해요" 질문이 있는데 답이 안 달렸다 / - v7: "오답노트 카드"가 내부 용어였다(11조). 학생 화면 문구는 "오답노트 한 장" / - v8: 원문 사진 2장 삽입 결정(기윤) — 접는 동작 1장 + 접힌 자리 메모 1장 / - v9: ⑤행동을 등판 앞으로 재배치 · 행동 문장 교체 · 제목 A 확정 / 에이전트 첫 독자 3회(v2.1·v3·v6, 매번 새 에이전트)에서 나온 것도 반영. / 두 번 연속 같은 자리에서 광고로 감지된 게 재배치의 근거다. / PROGRESS.md는 앞선 커밋 4건의 자동 로그(log:commit 산출물). / Co-Authored-By: Claude Opus 5 <noreply@anthropic.com>
...
- 원격: origin
- 원격 URL: https://github.com/kiyounpark/dasida-app.git
- 링크: https://github.com/kiyounpark/dasida-app/commit/b9b16472e0acfeca533109566a3537cea8106800
- 작성자: 박기윤
- 메시지: chore: 수학 선생님 에이전트 — 약점 판정 검수용
- 본문: 문항을 만드는 게 아니라 약점이 잘 잡히는지를 본다 (08.14 기윤 결정). / ① 어긋났나 = 이름표 가리고 맞힌 뒤 대조 / ② 어색한가 = 보여주고 따지기 / 부르는 법이 반대라 섞지 않는다. 각주 = 풀이, 못 대면 판정을 버린다. / 첫 실전 판정: "고치면 쓸만함" / - 수학 지어내기 0건 (채점자가 계산 40개 재검산) / - 이미 아는 4건 전부 잡음 + 새 발견 8건 / - 흠: 없는 인용 1건 · 잣대 불균일 · "없다"가 0개 / Co-Authored-By: Claude Opus 5 <noreply@anthropic.com>

### 커밋 2026.08.14 00:44
- 해시: `3a876b5` (`3a876b5a5d603e9437695de38eac6531501be733`)
- 브랜치: main
- 원격: origin
- 원격 URL: https://github.com/kiyounpark/dasida-app.git
- 링크: https://github.com/kiyounpark/dasida-app/commit/3a876b5a5d603e9437695de38eac6531501be733
- 작성자: 박기윤
- 메시지: chore: 에이전트 둘 — 수학 검산기·타겟 학생
- 본문: 고3 트리 68칸을 채우려면 새 약점 이름표마다 5지선다 문항까지 써야 하는데, / 그걸 검수할 사람이 없다는 게 08.13 밤에 드러났다. 그 자리를 메우는 정의 둘. / - math-checker: 정답을 안 받고 직접 푼 뒤 저장된 답과 대조. / 계산은 코드로만(암산 금지). 만드는 쪽과 반드시 분리. / - target-student: 화면 문구가 타겟한테 걸리는지 본다. / 판정은 하되 오르비 원문을 붙인다 — 못 대면 그 답은 버린다. / 08.02 "에이전트는 판정자가 아니다"를 각주 조건부로 푼 것 / (07.31 사고는 각주 규칙 08.08 이전이었다). / 수학 선생님(문항 만드는 쪽)은 초안까지만. 자동화 방법도 아직. / Co-Authored-By: Claude Opus 5 <noreply@anthropic.com>

### 커밋 2026.08.14 00:34
- 해시: `6702b61` (`6702b61aca048bafb67a4999062ef2440082f641`)
- 브랜치: main
- 원격: origin
- 원격 URL: https://github.com/kiyounpark/dasida-app.git
- 링크: https://github.com/kiyounpark/dasida-app/commit/6702b61aca048bafb67a4999062ef2440082f641
- 작성자: 박기윤
- 메시지: docs: 오르비 10호(2편) v3·v4 — 예고 확정 + 3편 형태 결정
- 본문: v3: 예고 후보 ㄱ/ㄴ 중 ㄴ(「어디서 멈췄나」) 확정 / v4: 예고 교체 — 「좋아요 1등 방법을 그대로 얹는다」 / ㄴ을 버린 이유는 재료 실측. 08.11 크롤 1,137편에서 새로 나온 건 / ①② 둘뿐이고(④ "새 방향 아님", ⑤ "중복"), ②는 "2건뿐, 한 편을 / 지탱하기엔 얇다"고 같은 문서가 판정해놨음. 얇은 재료 대신 본문 / 다섯 편(합 조회 75,383)을 3편 재료로 넘김. / 3편 형태 = 개발 기록 + 학생 주제. 표본 0인 자리라 내기. / 본문은 v1 이후 한 글자도 안 건드림 (08.11 "2편은 셋 다 유지"). / Co-Authored-By: Claude Opus 5 <noreply@anthropic.com>
...
- 원격: origin
- 원격 URL: https://github.com/kiyounpark/dasida-app.git
- 링크: https://github.com/kiyounpark/dasida-app/commit/f224009ed15c93bad95767b0378e2765f6b7037a
- 작성자: 박기윤
- 메시지: Merge pull request #29 from kiyounpark/fix/android-push-priority-channel
- 본문: fix(notifications): Android 푸시 페이로드에 priority/channelId 추가
...
- 2026-05-21: Android FCM 검증 완료 + 푸시 페이로드 회귀 fix (PR #29). 어제(05-19) spec §8 핸드오프 후속. 작업 흐름: (1) EAS dev 프로필(`com.dasida.app.dev`)에 FCM V1 서비스 계정 키 할당(production용으로 어제 올린 키 재사용 — Expo는 applicationId 단위 자격 분리), (2) Firebase 콘솔에서 dev 앱에 진짜 빌드용 SHA-1/SHA-256 등록 → `google-services.json` 동기화 → Google Sign-In `DEVELOPER_ERROR` 해소, (3) `APP_VARIANT=dev` prebuild → 무선 adb로 갤럭시 S10 5G에 dev-client 빌드/설치, (4) 결과 화면에서 토큰 등록 트리거 확인(약점 1개+ + 권한 grant 조건), Firestore `users/{accountKey}.pushTokens`에 android 토큰 박힘 확인, (5) Expo Push에 직접 토큰 단건 발송 — 첫 시도 `InvalidCredentials`(production만 키 연결됐던 흔적) → dev 슬롯 키 연결 후 ok + 폰 수신 확인. **회귀 발견 + fix**: 운영 함수 `sendReviewReminders` 페이로드가 `priority`/`channelId`를 비워 보내면 Expo Push 영수증은 `ok`인데 Android가 저우선 채널로 묶어 화면 표시를 묵음 처리(iOS 무영향, 채널 개념 없음). `buildPushMessages` 순수 함수로 추출(`review-reminder-core.ts`) — 모든 토큰에 `priority: 'high'`, `channelId: 'default'` 일괄 부여. `'review'` 채널은 클라 게스트 로컬 스케줄 경로에서만 생성되며 인증 사용자 서버 경로에서는 보장되지 않으므로 Expo 자동 생성하는 `'default'`로 고정(주석에 명시). 페이로드 누락 회귀 테스트 2건 추가. superpowers TDD(RED→GREEN→REFACTOR) + code-reviewer 서브에이전트(Important 1건: 채널 선택 근거 인라인 주석 보강 반영). functions 60/60 그린, tsc 0. 배포 후 `runReviewReminders('evening')` 직접 호출로 새 코드 경로 검증 — iOS·iPad·Android 3대 모두 동일 카피 수신·표시 확인. 종료: PR #29 머지 + main 배포 (`sendReviewRemindersMorning`/`Evening`).
- 2026-07-25: 즉석 재도전 + 개인화 망각곡선 엔딩 (web-proto, 브랜치 `feat/instant-retry-curve`, base `feat/photo-flow-error-pointing` 스택). 사다리가 약점 카드 뒤 IOU("앱에서 이어서 잡아줄게")로 끝나 진단만 있고 갚는 장면이 없던 문제 — 학생이 만족하는 순간은 "약점을 알았다"가 아니라 "아까 무너진 그 자리를 이번엔 통과했다". **서버**: `analyzePhoto` 원샷 응답의 `errorCandidates[i]`에 선택 필드 4종(`retrySetup`/`retryPrompt`/`retryOptions`/`retryAnswerIndex`) 추가 — 새 API 호출 0(원샷 유지). 조교 규칙서 11번 신설(틀린 그 단계만·같은 원리·숫자만 변경·새 개념 금지), `sanitizeErrorCandidates`에 retry 검증 추가하되 **불량이어도 후보는 살리고 retry 4필드만 제거**(짚기·쪽지시험 정상 진행). **웹**: `startRetry`(if 관문 — retry 필드 없으면 조용히 건너뛰고 곡선 착지, 넘어갈래 ghost 탈출구, 오답 시 정답 한 줄만·재시도 없음), `showForgettingCurve`(variant 3종 success/fail/survey, SVG 곡선 + 방금 잡은 약점을 `{방법} × {실수유형}` 라벨로 곡선 위에 얹음), 스토어 CTA 2버튼. **계획서 오류 2건을 구현 중 정정**: (1) 계획서는 retry를 `required`에서 빼라고 했으나 스키마가 `strict: true`라 모든 property가 required여야 함 — `npm run build`로는 안 잡히고 실호출에서 400으로 즉사했을 것. `userAnswer`의 기존 nullable-required 패턴(`type: ['string','null']`)으로 전환하고 프롬프트도 "생략" → "4개 모두 null"로 동반 수정. (2) 계획서가 CSS를 `web-proto/styles.css`에 넣으라 했으나 `index.html`에 `<link>` 태그가 0개라 그 파일은 죽은 파일(클래스명도 `.bubble.assistant` vs 실제 `.bubble.coach`로 불일치) — 인라인 `<style>`로 넣음. **리뷰에서 나온 회귀 fix**: 375px에서 액션 버튼이 화면 밖으로 완전히 이탈(재도전 −276px, CTA −165px). 원인은 `setActions`가 자기 행을 스크롤하지 않아 말풍선 기준 스크롤이 항상 버튼 행을 `viewport_bottom + 24`에 park시키던 것 — 신규 화면만이 아니라 **기존 쪽지시험 화면도 이미 버튼 0개 노출 상태였음**. `setActions`에 `scrollIntoView({block:'nearest'})` 추가로 6개 화면 전부 해소(`'end'`는 31버튼짜리 `showAllMethods`에서 질문을 1170px 밀어내 기각). iPadOS 13+ Safari가 UA를 `Macintosh`로 보고해 아이패드 사용자가 Play 스토어로 가던 버그도 `maxTouchPoints` 가드로 fix. retry 인덱스 범위 검사 추가(하드코딩 `3` 제거, 배열 길이에서 유도). superpowers subagent-driven-development(구현 4 + 스펙리뷰 1 + 코드리뷰 2) — 리뷰어가 컴파일된 정화기를 실제 변조해 회귀 테스트 3종이 무력함을 입증, 인덱스 밀림·원자성·스키마 strict 불변식 테스트로 봉함. functions 82/82 그린(79→82), tsc 0, `node --check` 통과. **미검증(수동 필요)**: 실사진 시식 — retry 문제의 정답 키가 실제로 맞는지는 구조적으로 정화 불가(규칙 11에 "속으로 검산" 조항은 추가). 스토어 링크 실동작. 문구 톤 검수 4건(카드 "문제 더 풀지 말고" 직후 재도전 문제 제시 모순, `-줘` 어미 명령/서술 중의성 등)은 기윤 몫으로 남김.
- 2026-09-07: 1.0.8 두 스토어 심사 제출 (1.0.8 계획표 15번 = 마지막 칸). **iOS**: 전날 EAS 빌드가 AppCheckCore 모듈 헤더에서 죽어 buildNumber 17이 `errored`였고, `00b6092`(google-signin 16.1.4)로 복구해 **18이 finished**(09.06 11:13). `eas submit --platform ios --latest`로 ASC 업로드 → App Store Connect에서 빌드 18 바인딩 → 심사 제출(`Waiting for Review`, 애플 고지 "It can take up to 48 hours to be reviewed"). ⚠️ **첫 Add for Review는 거부됨** — `Korean - What's New in This Version - This field is required`. 업데이트 버전은 릴리스 노트가 필수인데 검증 전에는 그 입력 칸이 화면에 드러나지 않아 놓치기 쉽다(다음 릴리스 체크리스트 항목). **Android**: `eas submit --platform android --latest`가 `eas.json`의 `track: "internal"`대로 내부 테스트에 올린 뒤 Play Console에서 프로덕션 승격 — 1.0.6(06.25 내부 20:39 → 프로덕션 21:15, 36분차)과 동일 경로라 `eas.json`은 수정하지 않음. 구글 고지 "검토는 일반적으로 7일 이내". 릴리스 노트는 두 스토어 동일 문구(사진 촬영/앨범 업로드 안내). ⚠️ **프로덕션 승격 화면 경고 2건**: (1) 1.0.8이 기기 405종을 더 이상 지원하지 않음 — 전화 67종(−1%) · 태블릿 267종(−4%) · 크롬북 62종(−86%) · 차량 8종(−100%) · TV 1종. Play Console 설치 수 0이라 기존 사용자 영향 0, **원인 미조사**(제출 우선). (2) R8/proguard 난독화 매핑 미첨부 — 크래시 분석 가독성 경고, 출시 비차단. 코드 변경 0 — 제출·스토어 작업만.
- 2026-09-08: web-proto 조사 괄호 fix + 번들 13일치 재빌드 (배포 완료, `dasida-proto.netlify.app`). 유튜브 영상에 링크가 나가 있어 지금 학생이 들어오는 화면. **① 조사**: `web-proto/app.js` 3곳(360·378·531)이 `(으)로`를 하드코딩해 "근의 공식(으)로 접근했네."가 그대로 나가고 있었다. 앱은 09.02 `23a119e`에서 `ro()`로 이미 닫은 자리 — 웹에 사본을 두지 않으려고 `flow-entry.ts`에서 `ro`를 export해 번들로 내보내고 `app.js`는 `const ro = F.ro;`로 받는다(`flow-entry.ts`가 diagnosisMap에 대해 이미 못 박아둔 "사본 금지" 방식). 31개 풀이법 라벨 전수 통과, 받침 양쪽 분기 확인(근의 공식**으로** / 인수분해**로**). **② 번들**: `88d7c1c`(08.13) 이후 안 말아서 밀려 있던 걸 재빌드. ⚠️ **esbuild가 devDependency에 없다** — `npx`는 rtk 훅에 막히므로 npx 캐시의 바이너리(`~/.npm/_npx/*/node_modules/.bin/esbuild`)로 빌드했다. 0.27.3/0.27.7/0.28.2 셋 다 08.13 소스에서 기존 번들을 **바이트 단위로 재현**해서 포맷 노이즈는 0. **59번째 약점(`g3_log_exp_base`)은 재빌드해도 안 붙는다** — `293942e`(1.0.7)가 `diagnosisTree` 갈래를 `g3_log_exp`로 되돌려놨고 `weaknessCandidatesFor`는 그 트리를 훑기 때문. 번들엔 이름만 들어오고 180개 (풀이법×실수유형) 조합 어디서도 안 나온다(브라우저에서 전수 확인). 서버 `weaknessOrder` 배포 뒤 `diagnosisTree` 한 줄 되걸면 살아난다. 재빌드가 실어 나른 실제 내용은 이름 1개가 아니라 `detailedDiagnosisFlows` 문구 교정 2건도 포함 — 다만 `app.js`가 번들에서 쓰는 건 `diagnosisMethodRoutingCatalog`·`ro`·`methodOptions`·`weaknessCandidatesFor`·`diagnosisMap` 5개뿐이고 `getDiagnosisFlow`/`getNode`/`advanceFrom*`는 **한 번도 안 부르므로** 화면 영향 0. 검증: 로컬(fetch 스텁으로 3자리 전부) + 라이브에서 실사진 1장 실제 `analyzePhoto` 호출 → "풀이 읽었어. … — 근의 공식으로 접근했네." 콘솔 에러 0. tsc 0. **⚠️ 배포 명령은 긴 형태가 필수**: `netlify deploy --dir=web-proto --prod --no-build --site ee644b12-b30c-44c3-91a6-efaa0f1afe9f`. 저장소 `.netlify/state.json`은 **다른 사이트**(`dasida-21`, `1eaa87cf…`)에 링크돼 있어서 `--site`를 빼면 엉뚱한 사이트로 나가고, `dasida-proto`의 build_settings가 비어 있어 `--dir`을 빼면 비대화형에서 **저장소 루트 전체(`.env`·`.env.local` 포함)가 공개 URL로 업로드된다**. 위 08.31 기록의 짧은 `netlify deploy --prod`는 따라 쓰면 안 된다. → **같은 날 `package.json`에 `deploy:proto` 스크립트를 박아 플래그를 손으로 칠 일을 없앴다: `npm run deploy:proto`.** (`deploy:web`이 아니라 `deploy:proto`인 이유 — 기존 `web` 스크립트가 `expo start --web`이라 `deploy:web`은 "엑스포 웹 배포"로 정반대로 읽힌다.) **미해결(별건, 이번 변경 아님)**: 앱에 조사 하드코딩 3곳이 더 있다 — `done-view.tsx:48` "…은 더 이상 약점이 아닙니다"(받침 없는 라벨 27/59 깨짐, 졸업 화면), `bridge-view.tsx:69` "…는 N일 뒤에 다시 만나요"(받침 있는 31/59, 브리지 화면마다), `diagnostic-screen-helpers.ts:80`(31개 중 16개). `23a119e`의 "features/ 전체에 0건"은 `(으)` 형태만 grep한 결과라 사실이 아니다. `ro()`는 재사용 불가(은/는은 ㄹ받침도 '은'이라 규칙이 다름) — `utils/korean.ts`(기존 `getKoreanSubjectParticle` 있음)에 은/는 헬퍼를 새로 붙여야 한다. 그 외: 웹은 아직 학생에게 `(HTTP 500)` 원문을 alert로 노출(app.js:309, 앱은 09.02에 닫음), `web-proto/styles.css`는 어디서도 참조되지 않는데 매 배포마다 함께 올라간다(index.html:12 인라인 `<style>`이 실제 스타일).
- 2026-09-23: **웹 엔딩 정리(배포) + E칸 — 사진 노트가 복습 과제가 된다(앱+서버, functions 배포)**. **① 웹**(`5bb5d7a`, `dasida-proto.netlify.app`): 틀린 문장 "사진으로 노트 만드는 건 아직 앱엔 없어"와 수요 조사 버튼 삭제, "다음에 막힌 풀이도 보내줘." 추가, 스토어 버튼 되살림(1.0.8 양 스토어 확인 — 애플 조회 API `1.0.8 · 09.07`, 플레이 페이지 `1.0.8`), GA `store_open`. `setActions`가 누르는 순간 버튼 줄을 비워서(`app.js:241`) 스토어 탭에서 돌아온 학생이 빈 화면을 보던 걸 로컬 클릭 확인에서 잡아 다시 그리게 고침. Fable 검수가 "🔔 앱에서는 이 타이밍에 다시 물어봐" 약속이 1.0.8에선 거짓(E 없음)임을 잡아 **기윤 A안**으로 두 줄을 1.0.9까지 사실인 말로 교체(⏪ 주석, E칸 끝나는 조건에 되돌리기를 묶음). `flow-bundle.js` 재빌드(`g3_seq_sum_term` 0→5건, `rtk proxy npx esbuild`). 배포본 두 파일이 로컬과 바이트 동일. **② E**(`9c4181a`): 계획은 Claude 초안 → Fable 검수(6개 반영: functions 선배포가 필수 전제 — 실제 학생은 전부 로그인이라 원격 store, `.catch` 필요, import 스키마도 같은 enum 등). `LearningSource`·서버 `learningSources`에 `'photo'`, `finishNote`에서 `primaryWeaknessId`가 있을 때만 `spawnMistakeReviewTasks(..., 'photo')`(sourceId = 노트 id), `reviewTaskStore`를 `app/photo.tsx`·dev 주소에서 내려줌. **계획 밖 하나**: 복습 중 틀린 사진 과제가 `'photo'` 줄기를 잇게(`use-review-session-screen.ts`) — 안 그러면 한 번 틀리면 사진 표시를 잃는다. `review-content-map.ts:120` b/2 "ax²+bx+c" → "x²+bx+c"(math-teacher·target-student 둘 다 이 안). 테스트: 사진 화면 E칸 6 · 스케줄러 2 · 복습 세션 2 · 주소 1 추가, 연결 줄을 끄면 3개가 깨지는 것 확인. 앱 jest 646 · functions 86 통과, tsc 양쪽 0. functions 19개 배포(09.19 `g3_seq_sum_term` enum 두 줄도 같이 올라감 — 기존 배포 여부는 모름). **시뮬레이터(실제 구글 계정·실제 AI·배포 서버)**: 판별식 뺄셈을 틀린 손글씨 풀이 → `quadratic` · `= 11` 짚음 → 노트 🏷 "판별식 계산 실수" → 과제 저장 경고 0 → 홈 복습 그래프에 새 막대 → 앱 재실행 뒤에도 유지(캐시 폴백 0 = 서버 값). **못 본 것**: 홈 "오늘 복습할 것" — 로그인 계정 홈 due는 서버가 서버 시계로 계산(`learning-history.ts:531`)해서 맥 시계 옮기기가 안 먹힌다. 09.24 아침 실제 계정으로 확인(알림 포함). 남은 것: 1.0.9 빌드·제출 → 뜨면 웹 두 줄 되돌리기.
- 2026-09-26: 1.0.9 두 스토어 심사 제출 (`527a3d6` 버전 한 줄). 실린 것: E칸(사진 노트 → 복습 과제, source `'photo'`) + 사용량 로그 앱 쪽(계정 헤더). 네이티브 패키지는 1.0.8 빌드 뒤 변경 없음. typecheck 0 · jest 656/656 → EAS 프로덕션 iOS 19 · 안드 8 둘 다 FINISHED → `eas submit`. **iOS**: ASC에서 1.0.9 만들고 What's New 채우고 빌드 19 바인딩 → `Waiting for Review` (이번엔 빈칸 거부 없음). **Android**: 내부 테스트 → 프로덕션 승격 → 검토 전송(관리형 게시 꺼짐). 경고는 1.0.8 때와 같은 R8 매핑 없음 하나. 릴리스 노트 🔒 "사진으로 만든 오답노트가 이제 복습으로 이어져요. / 다음 날 홈에 그 약점 복습이 떠요." — target-student(Haiku)는 "찍기만 하면 알아서" 안을 골랐지만 과장이라 뺐다(과제는 AI 짚기 갈래 노트만). 콘솔 조작은 Claude가 크롬으로 직접 함(기윤 요청).
- 2026-09-26 (이어서): 안드 1.0.9 같은 날 게시 확인(콘솔 프로덕션 최신 1.0.9). 구글 스토어 설명 새로 씀 — 옛 글은 "기출 풀이 앱"이라 사진 오답노트가 0줄이고 없는 기능(그래프·가형/나형)을 적고 있었다. 기능 목록은 Explore 에이전트가 파일:줄로 뽑고 숫자(60회분·1/3/7/30일·07:30)는 Claude가 직접 셈. target-student(Haiku)가 "같이 찾고"(대화형 AI로 읽힘)·복습 줄(모든 노트가 복습되는 것처럼 읽힘)·이모지를 짚어 고침. "쓰는 게 귀찮을 뿐이죠"는 브랜드 축이라 남김. 간단한 설명 42자·자세한 설명 568자, 검토 전송. 애플 설명엔 지운 "10문항 진단"이 아직 있다 — 다음 버전 때. 콘솔 알림 "9월 30일까지 Android 개발자 인증"은 콘솔 배너에 "모든 Google Play 앱이 등록되었습니다"라고 적혀 있어 Play로만 내는 다시다는 할 것 없음.
- 2026-09-26 (이어서): 아침 알림(07:30)과 홈 "오늘 할 복습" 날짜 맞춤 — 서버만, `a3cce3c`, functions 배포. 서버가 만든 day3·7·30은 끝낸 시각 그대로라 그 시각 전엔 홈에 안 떴다(저녁 8시에 끝냈으면 D+3 저녁 8시까지). 서버 `addDays`를 KST 날짜 + N일 → `T00:00:00.000Z`(앱 관례)로, due 판정을 앞 10글자 <= 오늘 KST로. 옛 데이터는 안 옮김(요약을 읽을 때마다 due 재계산). 재현 테스트 4개 먼저 쓰고 실패 확인 → 고침 → functions 129/129. astra·Fable 리뷰 둘 다 "반드시 고칠 것 없음". 기록만: 밀린 과제만 있는 학생은 알림이 안 온다(원래부터).
