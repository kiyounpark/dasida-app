...
이 배치를 누가 왜 정했는지, 그리고 **결정 자체를 어떻게 내리는지**(astra·Fable)는
`docs/how-we-decide.md`에 있다.
...
### 🔒 단, 범위 밖 줄은 그 줄만 버린다 (2026.09.20 · Fable 결정)

> **만든 쪽이 지정 범위 밖 줄을 건드리면 그 줄만 버린다. 세트는 안 버린다.**

비우는 건 **세트가 깨졌을 때**다. 만든 쪽이 시키지도 않은 줄을 건드렸다가 그 줄이 깨진 건
세트가 깨진 게 아니다. 그 편집만 안 받고 나머지를 넣는다.

09.20 수열 판에서 두 번 나왔다. 두 번째가 `practiceMap.ts:445` `explanation`의
`이 식은` → `2n+3은`이었다 — 지정한 네 줄에 없던 편집이었고 거짓이 됐다
(다항식 `2n+3`은 n=1에 넣을 수 있다. n≥2에서만 성립하는 건 `aₙ=2n+3`이라는 **등식**이다).
그 줄만 버리고 나머지 넷 + 복습 세트를 넣었다.

⚠️ **빠져나가는 구멍으로 쓰지 마라.** 「범위 밖」은 **일감에 적어 준 줄 목록 밖**이라는 뜻이지
"검산이 깬 줄" 아무거나가 아니다. 범위는 일감을 보낼 때 정해져 있어야 한다.
