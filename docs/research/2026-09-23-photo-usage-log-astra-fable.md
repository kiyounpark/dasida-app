...
### "더 할 게 있나" 판 (09.24, 기윤 요청)

같은 질문을 둘에 걸었다 — "사진 분석 사용량 로그는 이제 더 안 해도 되나?" 둘의 답이 같아 최종 판은 안 돌렸다.

- **astra** (57,737 토큰): 로그 필드 추가는 권하지 않는다. 1.0.9 전엔 버전 변경 + 수동 판정 방식 확정. 재촬영은 사람이 확인하되, 같은 문제라고 무조건 빼지 말고 실제 재사용인지 본다. (a)는 기록된 AI 비용만(누락 usage·재시도 제외), (b)(c)는 후보까지만 센다
- **Fable** (159,520 토큰): 1.0.9 전에 코드로 더 할 건 없다 — 버전 한 줄뿐. 재촬영은 더 저장하지 않고 사람이 확인(3명 = 인터뷰 표본, `:813`). 서버 전용이라 "뜨기 전" 게이트가 아니다. 새로 찾은 것: "두 번째" 정의가 문서마다 갈린다(`:804` 사진 올림 vs `:722` 노트 생성 완료) — 첫 집계 전에 못 박는다. 1.0.9가 뜬 날 스토어 빌드로 1장 보내 `qa:false`·`appVersion:"1.0.9"` 확인

Claude 확인: 마케팅 문서 `:722`·`:804`·`:813`, `web-proto/app.js`의 `participant` 0건, `scripts/photo-usage-report.ts` 없음 — 적힌 대로.
