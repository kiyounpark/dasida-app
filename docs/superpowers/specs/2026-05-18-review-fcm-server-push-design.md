...
- 동작:
  ...
  2. **collectionGroup 쿼리**: `collectionGroup('reviewTasks')`
     ...
     `where('scheduledFor','>=', 오늘0시KST→UTC ISO)`
     `where('scheduledFor','<', 내일0시KST→UTC ISO)`.
     (`scheduledFor`는 1단계에서 `z.string().datetime()` ISO 저장 →
     ISO 문자열 사전식 범위 비교 유효.)
...
- **KST/UTC 경계 오류**: "오늘"을 KST 0시~내일 0시로 잡고 UTC ISO로
  환산해 쿼리. 순수 함수로 분리해 경계값 단위 테스트(7:30/20:00 KST,
  자정 인접 task). 사용자 전원 KST 수험생 가정 — 스펙 명시.
...
- 코드 대조: `scheduledFor` ISO 저장(`learning-history.ts:206,254`),
  1단계 인증 패턴(`learning-history-auth.ts`), 로컬 스케줄 진입점
