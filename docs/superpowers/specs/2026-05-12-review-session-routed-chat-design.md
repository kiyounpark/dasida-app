...
원래 비목표에 있던 항목 **"Remedial-flow 데이터 구조 자체의 변경 — 노드(`explain` / `check` / `exit`)는 유지"는 폐기**한다.

- **배경:** 복습 세션의 역할을 "오답 빠른 교정"에서 **"약점 학습 — 한 약점을 이번 세션에서 뿌리뽑기"**로 재정의 (사용자 결정 2026-05-13).
- **결과:** 보완 흐름(remedial-flow)이 짧은 1-shot에서 깊은 다단계 흐름(진단→설명→확인→재진단→정리)으로 확장됨. 기존 3종 노드만으로는 표현 불가.

#### 노드 종류 확장

**기존 3종 (유지):**
- `explain` — 설명 카드
- `check` — 확인 문제 카드 (정답/오답 분기)
- `exit` — 보완 흐름 종료, 본 thinkingStep 복귀 신호

**추가 2종 (본 결정으로 허용):**
- `diagnose` — **학생 사유 진단 카드.** "왜 [가]를 골랐어요?" 같이 정답·오답 없이 학생의 misconception 유형만 묻고 사유별로 다른 분기로 보냄. explain은 설명만, check는 정답/오답 판정만 — diagnose는 둘 다 아닌 진단 전용.
- `summary` — **핵심 정리 카드.** 보완 흐름 마지막에 학습 요점 정리. explain과 달리 "여기까지 왔다"는 성취 신호 (UI 차별화 — 별 아이콘/다른 배경 등). 학생이 "이해됐어요" 누르면 exit으로.

**현재 결정 범위: 위 2종만 추가 허용.** 그 외 신규 종류(예: `step-reveal` — 단계별 풀이 펼치기)는 시범 작성 결과에 따라 별도 결정.

#### 깊은 보완 흐름 예시 (`calc_repeated_error` 적용)
...
[check]     "f(-3) 의 답은?"
   ...
   └ 오답 → [diagnose]  "어디서 막혔어요?"
             ...
             [check]    재시도
       ↓
[summary]   "오늘 핵심: 음수 대입 시 괄호 필수. 잘했어요!"
       ↓
[exit]
```
...
- 보완 흐름이 깊어지면 학생이 폴백 챗에 의존할 빈도는 줄어듦
- 단, 자유 입력은 항상 가능하고 라우터/AI 폴백은 그대로 유지
- 폴백 2턴 제한은 유지 — 보완 흐름 자체가 깊어졌으므로 폴백을 더 풀 필요 없음

#### 영향 받는 문서

- [2026-05-13 remedial-content-pipeline-design](./2026-05-13-remedial-content-pipeline-design.md) — 본 결정에 따라 §4 파이프라인·§7 분량 제약·§9 일괄 실행 전부 갱신 필요
