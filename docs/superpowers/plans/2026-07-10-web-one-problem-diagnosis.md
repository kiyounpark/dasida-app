...
**Architecture:** 저장소 루트 `web/` 아래 정적 파일 4개(HTML/CSS/JS/데이터)로 구성된 단일 페이지. 서버·로그인·빌드 도구 없음. 화면 상태 4개(문제 → 풀이 선택(오답) / 지도(정답) → 진단 → 마무리)를 순수 JS로 전환. 데이터는 앱의 `data/exam/g3-calc-mock-2026-06/problems.json` 태깅에서 추출해 하드코딩.
...
- 참조: `data/exam/g3-calc-mock-2026-06/problems.json` (21번: 단답형, 정답 11, 극한·연속, diagnosisMethods `["limit"]`)

- [ ] **Step 1: 실제 문제·해설 확인**

`web/assets/problem-21.webp`를 열어 문제를 읽고, 기윤이 가진 6월 모평 해설(약점 태깅 검증에 쓴 해설지)과 대조한다. 아래 Step 2의 단계 지도 초안은 **극한·연속 유형의 일반 구조로 쓴 초안**이므로, 실제 21번 풀이 흐름에 맞게 문구를 고친 뒤에만 다음 단계로 간다.

- [ ] **Step 2: `web/problem-data.js` 작성 (초안 — Step 3에서 기윤 확정)**
...
window.PROBLEM = {
  ...
  // 풀이 단계 지도 (앱 약점 태깅 + 해설 기준)
  steps: [
    { name: '조건 해석', desc: '문제가 요구하는 조건이 무엇인지 정확히 읽는다.' },
    { name: '극한 조건 → 함수 성질 번역', desc: '주어진 극한 조건을 함수의 성질(연속·발산·수렴)로 바꾼다.' },
    { name: '케이스 나누기', desc: '조건을 만족하는 경우를 빠짐없이 나눈다.' },
    { name: '값 계산·합산', desc: '각 케이스의 값을 구해 답을 만든다.' },
  ],

  // 오답자용: "어떻게 풀었어요?" 선택지 → 어긋난 단계
  approaches: [
    { label: '조건이 뭘 요구하는지부터 헷갈렸다', brokenStep: 0,
      comment: '첫 단계에서 어긋났어요. 이 문제는 조건 해석이 반이에요.' },
    { label: '극한 조건을 보고 바로 식부터 세워 계산했다', brokenStep: 1,
      comment: '극한 조건을 함수 성질로 번역하는 단계를 건너뛰었어요. 여기가 이 문제의 발상 지점이에요.' },
    { label: '케이스를 나눴는데 일부를 빠뜨린 것 같다', brokenStep: 2,
      comment: '케이스 누락이에요. 조건을 만족하는 경우의 경계를 다시 보세요.' },
    { label: '풀이는 맞았는데 마지막 계산에서 틀렸다', brokenStep: 3,
  ...
  commonBreak: {
    stepIndex: 1,
    comment: '맞히셨네요. 근데 이 문제, 사람들은 대부분 여기서 무너져요 — 극한 조건을 함수 성질로 번역하는 단계요.',
  },

  sourceNote: '※ "무너지는 단계"는 실제 방문자 통계가 아니라 다시다 약점 태깅·해설 분석 기준입니다.',
};
```

- [ ] **Step 3: 기윤 검수 (게이트)**

기윤에게 `steps` / `approaches` / `commonBreak` 문구를 보여주고 실제 21번 해설과 맞는지 확정받는다. **확정 전에는 Task 3으로 넘어가지 않는다.** (앱 데이터 검증과 같은 기준 — 정직 원칙)

- [ ] **Step 4: Commit**

```bash
git add web/problem-data.js
git commit -m "feat(web): 6모 수학 21번 문제 데이터 + 단계 지도 (기윤 검수 완료)"
```
