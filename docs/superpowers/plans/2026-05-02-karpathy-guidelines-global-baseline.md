...
```
글로벌 셋업 적용 완료. 검증은 두 단계:

A. 객관적 검증 (즉시, falsifiable)
   1. 새 Claude Code 세션 시작 (이 세션 종료 후 또는 다른 프로젝트)
   2. 첫 메시지로: "지금 너에게 로드된 글로벌 instruction 파일들의 첫 줄을 각각 출력해줘"
   3. 응답에 `# Karpathy Guidelines`(또는 `# CLAUDE.md`) 라인이 포함되면 import 성공.
      포함 안 되면 → cat ~/.claude/CLAUDE.md로 직접 확인, 줄 빠졌으면 다시 추가.

B. 행동 검증 (며칠~몇 주 누적 관찰)
   - 가정을 명시하거나 명확화 질문을 던지는가?
   - 무관한 코드 리팩터링을 자제하는가?
   - 검증 가능한 성공 기준을 제시하는가?
   행동 검증은 단일 세션에서 falsify 어려움 — 누적 관찰로 판단.
```
