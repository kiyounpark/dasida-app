...
**선행 개념도 풀어쓰기 대상**: 본 약점이 다루는 핵심 용어뿐 아니라, 그 설명 중에 자연스럽게 등장하는 **선행 개념**도 첫 등장 시 한 줄 정의 동봉. 예시:

- "소수" → "1과 자기 자신으로만 나뉘는 수 (예: 2, 3, 5, 7)"
- "소인수분해" → "작은 소수들의 곱으로 쪼개기"
- "약분" → "분자·분모를 같은 수로 나눠 줄이기"
- "계수" → "변수 앞에 곱해진 수"

기준: 약점 정의에 직접 등장하지 않더라도, 학생이 본문을 읽다가 "이게 뭐였더라?" 멈출 만한 용어면 풀어쓰기.

근거: pilot 결과 학생 게이트가 "소수", "소인수분해", "약분" 같은 선행 개념에서 반복 confused 발생. 풀어쓰기 동봉으로 1차 통과율 ↑.
