...
다음 패턴은 sympy로 보내지 말 것 (조건 하에서만 참이라 sympy가 reject):

- `(x-a)² + b = b` — x=a 조건에서만 참 (변수 a, b는 자유 변수로 잡혀 false 처리됨)
- `√(a²·b) = a√b` — a≥0 조건에서만 참 (a가 음수면 |a|√b)
- 정의문 (예: "x=a일 때 최솟값") — 정의는 등식 검증 대상이 아님
- 부등식 ((x-a)² ≥ 0 등)

원칙: equationsToVerify에 넣는 등식은 **모든 자유 변수에 대해 항등식이거나, 모든 변수가 명시된 상수로 치환된 산술 등식**이어야 함. 예:

- ✅ `(-2)² = 4` — 상수 등식
- ✅ `(x+a)² = x² + 2ax + a²` — 전개 항등식 (모든 a, x에 대해 참)
- ❌ `(x-a)² + b = b` — 조건부 (x=a일 때만)
- ❌ `√(x²) = x` — 조건부 (x≥0)

조건부 식은 issues 필드에 minor로 기록하거나 무시. equationsToVerify에 넣지 말 것.
