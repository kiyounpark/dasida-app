...
exports.detailedDiagnosisFlows = void 0;
const diagnosisMap_1 = require("./diagnosisMap");
const diagnosisTree_1 = require("./diagnosisTree");
const CONTINUE_LABEL = '확인 문제로 넘어갈게요';
const DONT_KNOW_LABEL = '모르겠습니다';
const FINAL_LABEL = '이 약점으로 정리하기';
const methodFallbackWeakness = {
...
const customExplainCopyByChoice = {
    cps_formula: {
...
function getMethodLabel(methodId) {
    return diagnosisTree_1.methodOptions.find((option) => option.id === methodId)?.labelKo ?? methodId;
}
function buildDefaultExplainCopy(methodId, choiceId, weaknessId) {
    ...
    const weakness = diagnosisMap_1.diagnosisMap[weaknessId];
...
function createMethodFlow(methodId) {
    const definition = diagnosisTree_1.diagnosisTree[methodId];
    const methodLabel = getMethodLabel(methodId);
    ...
    definition.choices.forEach((choice) => {
        ...
        if (hasCheckNode) {
            ...
            nodes[`${choice.id}_check`] = buildCheckNode(`${choice.id}_check`, choice.weaknessId, finalNodeId, `${choice.id}_remedial`);
            nodes[`${choice.id}_retry_check`] = buildCheckNode(`${choice.id}_retry_check`, choice.weaknessId, finalNodeId, fallbackFinalNodeId);
        }
        nodes[finalNodeId] = buildFinalNode(finalNodeId, choice.weaknessId);
        nodes[fallbackFinalNodeId] = buildFinalNode(fallbackFinalNodeId, fallbackWeaknessId, `${methodLabel} 풀이의 기초를 먼저 다지는 것이 좋아 보여요.`);
    });
    return {
        methodId,
        startNodeId: 'root',
...
const flowEntries = diagnosisTree_1.methodOptions.map((option) => [option.id, createMethodFlow(option.id)]);
exports.detailedDiagnosisFlows = Object.fromEntries(flowEntries);
