...
exports.buildHomeJourneyState = buildHomeJourneyState;
...
function hasValidPendingResume(profile, summary) {
    ...
    if (!pending.attemptId)
        return false;
    if (pending.diagnosisQueue.length === 0)
        return false;
    if (summary.latestDiagnosticSummary?.attemptId === pending.attemptId)
        return false;
    return true;
}
function isPendingDiagnosticFresh(profile, summary) {
...
function isPendingPracticeFresh(profile, summary) {
...
function hasActivityAfter(summary, kind, timestamp) {
    if (!timestamp) {
        return false;
    }
...
function getWeaknessLabel(summary) {
    const weaknessId = summary.latestDiagnosticSummary?.topWeaknesses[0];
    if (!weaknessId) {
        return '최근 약점 없음';
    }
    return diagnosisMap_1.diagnosisMap[weaknessId].labelKo;
}
function getCurrentState(summary, profile) {
    // 7: 졸업은 항상 최우선.
...
function getStepStatus(currentStep, step) {
...
function getStepDetail(step, summary) {
    if (step === 'diagnostic') {
...
function getStepStatusLabel(step, status, summary) {
    if (status === 'completed') {
        return '완료';
    }
    if (status === 'locked') {
...
function getCurrentStepBody(currentStepKey, summary) {
...
function buildHomeJourneyState(summary, profile) {
    ...
    const stepOrder = ['diagnostic', 'analysis', 'review', 'exam'];
